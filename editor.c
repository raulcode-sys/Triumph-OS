/*
 * editor.c — Triumph OS built-in text editor
 *
 * A nano-style terminal text editor, compiled directly into the shell.
 * Include this file from triumph.c with:  #include "editor.c"
 *
 * Keybindings:
 ...
 *   Ctrl-G           — help bar toggle
 *   Ctrl-A / Home    — start of line
 *   Ctrl-E / End     — end of line
 *   Ctrl-F / PgDn    — page down
 *   Ctrl-B / PgUp    — page up
 *   Ctrl-L           — refresh screen
 *   Ctrl-W           — find (search forward)
 *   Ctrl-N           — find next
...
/* ── Types ────────────────────────────────────────────────────────────────── */
typedef struct {
    char  *buf;       /* line content (heap, nul-terminated) */
    int    len;       /* length not including nul             */
...
typedef struct {
    ...
    int     cx, cy;       /* cursor col, row (0-based in file)  */
    int     scroll_row;   /* first visible row                  */
    int     scroll_col;   /* first visible col                  */
...
/* ── Terminal helpers (raw mode assumed already off during editing) ────────── */
static void ed_term_size(void) {
    struct winsize ws;
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == 0 && ws.ws_row && ws.ws_col) {
        E.rows = ws.ws_row - 3;   /* reserve 3 lines: header + status + keys */
        E.cols = ws.ws_col;
    } else {
        E.rows = 22; E.cols = 80;
    }
...
/* ── Write helper (avoids buffering issues) ───────────────────────────────── */
...
static void ed_render(void) {
    ...
    /* ── header bar ── */
    {
        ...
        ab_appends(&ab, "\x1b[48;5;17m\x1b[38;5;51m\x1b[1m");  /* dark-blue bg, cyan fg */
    ...
    {
        char cur[32];
        int screen_row = (E.cy - E.scroll_row) + 2;   /* +2 for header + 1-index */
        int screen_col = (E.cx - E.scroll_col) + 6;   /* +5 for line num, +1 for 1-index */
        snprintf(cur, sizeof(cur), "\x1b[%d;%dH", screen_row, screen_col);
        ab_appends(&ab, cur);
    }

    ab_appends(&ab, "\x1b[?25h");   /* show cursor */
...
/* parse mouse click from SGR or X10 escape, move cursor to clicked position */
static void ed_handle_mouse(int ox, int scroll_row, int scroll_col) {
    /* read rest of mouse escape: already consumed ESC [ M or ESC [ < */
    ...
    buf[bi] = 0;
    /* SGR format: btn;col;rowM or btn;col;rowm */
    int btn, mx, my;
    if (sscanf(buf, "%d;%d;%d", &btn, &mx, &my) == 3) {
        /* convert screen coords to file coords */
        int fy = (my - 2) + scroll_row;   /* -2 for header row, 1-indexed */
        int fx = (mx - 6) + scroll_col;   /* -5 for line numbers, -1 for 1-index */
        if (fy >= 0 && fy < E.nlines) {
            E.cy = fy;
...
static void ed_move(int key) {
    switch (key) {
        ...
        case ED_KEY_ARROW_DOWN:
            ...
            ed_clamp_cx();
            break;
        case ED_KEY_HOME:  case 1:  /* Ctrl-A */
            E.cx = 0; break;
        case ED_KEY_END:   case 5:  /* Ctrl-E */
            if (E.cy < E.nlines) E.cx = E.lines[E.cy].len; break;
        case ED_KEY_PGUP:  case 2:  /* Ctrl-B */
            E.cy -= E.rows;
            if (E.cy < 0) E.cy = 0;
            ed_clamp_cx();
            break;
        case ED_KEY_PGDN:  case 6:  /* Ctrl-F */
            E.cy += E.rows;
...
static int b_edit(Cmd *cmd) {
    ...
    ed_puts("\x1b[?1000h\x1b[?1002h\x1b[?1015h\x1b[?1006h"); /* enable mouse */
    ...
    ed_puts("\x1b[?1006l\x1b[?1015l\x1b[?1002l\x1b[?1000l"); /* disable mouse */
    ed_puts("\x1b[2J\x1b[H");
